	...
	<div style="padding-bottom: 1.31rem">
		<div class="weight">
			...
			<div class="block">
				...
				<div class="weightVal">
					<div class="jiLw" @click="recordBlood()">记录心率</div>
				</div>
		...
		methods: {
			bloodheartRateValue() {
				get('/health-web/modules/userDataRecord/latestInfo/heart_rate', {}).then((res) => {
					if(res.code == 0) {
						this.value2 = res.userLatestRecord.heartRateValue
					}
				})
			},
			//点击记录血糖调接口**
			recordBlood() {
				...
				get('/health-web/modules/userDataRecord/latestInfo/heart_rate', params).then((res) => {
					if(res.code == 0) {
						Toast({
							message: '记录血糖成功',
							duration: 1500
